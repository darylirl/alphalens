...
Any paginated or capped read must report whether it hit its limit. A ceiling
that returns partial data as if complete is the most dangerous failure mode in
this codebase — it has now produced three wrong figures (PostgREST 1,000-row
cap, RPC row cap, a 30-page fill walker). Every reader returns an explicit
total and a truncation flag; callers must treat an unflagged partial read as a
bug, not a result.
