    ...
    <div className="space-y-3">
      <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
        ...
        <button
          ...
          className={`ml-auto flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full transition-colors ${
            expanded ? 'bg-[#34EAB920] text-[#34EAB9]' : 'bg-[#0F1A1E] text-white/55'
          }`}
        >
          {expanded ? <X size={12} /> : <SlidersHorizontal size={12} />}
          {expanded ? 'Close' : 'Filters'}
          {activeFilterCount > 0 && !expanded && (
            <span className="bg-[#34EAB9] text-[#0F1A1E] text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center">
              {activeFilterCount}
            </span>
          )}
        </button>
      ...
      {expanded && (
        ...
        >
          {/* Sort options */}
          <div>
            <p className="text-[10px] uppercase tracking-wider text-white/40 mb-2">Sort By</p>
                ...
                <button
                  ...
                  className={`text-xs px-3 py-1.5 rounded-full transition-colors ${
                    sort === s.value
                      ...
                      : 'bg-[#0F1A1E] text-white/55 border border-transparent hover:text-[#F0FAF8]'
                  }`}
                >
                  {s.label}
                </button>
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs text-white/55 hover:text-[#F0FAF8] transition-colors"
          >
          ...
          {showAdvanced && (
            ...
            >
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                <div>
                  <label className="text-[10px] uppercase tracking-wider text-white/40 mb-1 block">Time Window</label>
                  <select
                    value={filters.timeWindow}
                    onChange={e => updateFilter('timeWindow', e.target.value)}
                    className="w-full bg-[#0F1A1E] border border-white/[0.08] rounded-lg px-2.5 py-2 text-xs outline-none focus:border-[#34EAB9]"
                  >
                    {TIME_WINDOWS.map(t => (
                ...
                <div>
                  <label className="text-[10px] uppercase tracking-wider text-white/40 mb-1 block">Min Trades</label>
                  <input
                    type="number"
                    value={filters.minTrades}
                    onChange={e => updateFilter('minTrades', e.target.value)}
                    placeholder="e.g. 10"
                    className="w-full bg-[#0F1A1E] border border-white/[0.08] rounded-lg px-2.5 py-2 text-xs outline-none focus:border-[#34EAB9] placeholder:text-white/40"
                  />
                </div>
                <div>
                  <label className="text-[10px] uppercase tracking-wider text-white/40 mb-1 block">Min Win Rate (%)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="text-[10px] uppercase tracking-wider text-white/40 mb-1 block">Min PnL (USD)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="text-[10px] uppercase tracking-wider text-white/40 mb-1 block">Max Leverage</label>
                  <input
                    type="number"
